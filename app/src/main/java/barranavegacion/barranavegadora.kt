...
@Composable
fun BarraNavegadoraSimple(tipoUsuario: String) {
    var pantallaSeleccionada by remember { mutableStateOf(0) }

    // Si es comun, solo mostrar portón
    if (tipoUsuario == "comun") {
        Box(
            ...
            Box(
                ...
                controlporton.ControlPortonScreen()
            }
            // Barra inferior solo con Portón
            Surface(
                color = Color.White,
                modifier = Modifier
        ...
        // admin: mostrar todas las pantallas
        Box(
            modifier = Modifier.fillMaxSize()
        ) {
            Box(
